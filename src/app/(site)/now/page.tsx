...
const completedProjects = [
  { title: "Peptides Calculator", href: "https://www.peptidescalculator.app/", label: "/application/" },
  { title: "My TRT App", href: "https://www.mytrt.app", label: "/application/" },
];

const pipeline = ["Updating Soon"];
...
const webAppStack = ["Pipedrive", "Zapier", "Lemlist", "Intercom", "Unbounce", "Outranking", "Neuraltext", "Nichesss", "Frase"];
const gear = ["Sony A7iii", "Glaxy Z Fold 2", "Apple iPhone 17 Max Pro", "iPad 6th Gen 2TB", "MacBook Pro M4 - 128GB RAM, 4 TB HD"];
  ...
  return (
    <ul className="grid gap-2 text-sm text-zinc-300 sm:grid-cols-2 sm:text-base">
      {filtered.map((item) => (
        <li
          key={item}
          className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-zinc-200"
        >
          {item}
        </li>
      ))}
        ...
        <Section id="completed" title="Completed Projects">
          <ul className="space-y-3 text-sm text-zinc-300 sm:text-base">
            {completedProjects.map((p) => (
              <li key={p.href} className="rounded-2xl border border-white/10 bg-white/5 p-4">
